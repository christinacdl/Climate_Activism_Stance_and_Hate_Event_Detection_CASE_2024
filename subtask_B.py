...
args = {'task_name': '=== Climate Activism Stance and Hate Event Detection Shared Task at CASE@EACL2024-SubTask B ===',
        'data_directory': '/home/ch.christodoulou/classification/Sub-task_B/',
        'results_data_directory': '/home/ch.christodoulou/classification/Sub-task_B/results/',
        'output_model_directory': '/home/ch.christodoulou/classification/Sub-task_B/outputs/',
        'model_name': str(dict_causal_model_names[8]), # Change the dictionary name and the index of the model of your choice
        ...
        'new_model_name': 'Mistral-PromptTuning-Hate-Target-Detection',
        ...
        'prompt_tuning_init_text': "[INST]Your task is to classify the target of hate speech as individual, organization or community, and return the answer as the corresponding label '0' or '1' or '2'[/INST]",
...
tokenizer = tokenizer_class.from_pretrained(args['model_name'], add_prefix_space=False,
                                            use_fast = True, trust_remote_code=True, add_eos_token=True, add_bos_token = True, 
                                            padding_side = 'left', do_lower_case = False) 
...
##==================== LABELS OF SUB-TASKS ====================##
label2id = {'INDIVIDUAL': 0, 'ORGANIZATION': 1, 'COMMUNITY': 2}
id2label = {0: 'INDIVIDUAL', 1: 'ORGANIZATION', 2: 'COMMUNITY'}
class_names = ['INDIVIDUAL', 'ORGANIZATION', 'COMMUNITY']

##==================== OPEN AND PROCESS DATASETS ====================##
training_data = open_dataset(args['data_directory'] + 'SubTask-B-train.csv', 
                            'tweet', 'label', huggingface_dataset = False, json_dataset = False, csv_tsv_dataset = True, 
...
# df1 = pd.read_csv(args['data_directory'] + 'SubTask-B(index,tweet)val.csv')
# df2 = pd.read_csv(args['data_directory'] + 'SubTask-B(index,label)val.csv')
# data = pd.merge(df1, df2, on='index')
# # print(data)
# data.to_csv(args['data_directory'] + 'SubTask-B_labelled_val.csv', index=False)

val_data = open_dataset(args['data_directory'] + 'SubTask-B_labelled_val.csv', 
                                  'tweet', 'label', huggingface_dataset = False, json_dataset = False, csv_tsv_dataset = True, 
...
# df1 = pd.read_csv(args['data_directory'] + 'SubTask-B(index,tweet)test.csv')
# df2 = pd.read_csv(args['data_directory'] + 'SubTask-B(index,label)test.csv')
# data = pd.merge(df1, df2, on='index') 
# data.to_csv(args['data_directory'] + 'SubTask-B_labelled_test.csv', index=False)

test_data = open_dataset(args['data_directory'] + 'SubTask-B(index,tweet)test.csv', 
                                  'tweet', None, huggingface_dataset = False, json_dataset = False, csv_tsv_dataset = True, 
                                   type_split = 1, clean_text= False, labelled_dataset = True)
...
encoded_train_dataset = train_dataset.map(
    ...
    batched=True,
    num_proc=1,
    remove_columns = ['text', 'level_0', 'index'],
...
def get_weighted_trainer(classes):
  ...
  weights_class_1, weight_class_2, weight_class_3 = compute_class_weights(classes)
  ...
  class _WeightedBCELossTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
      ...
      loss_fct = torch.nn.CrossEntropyLoss(weight=torch.tensor([weights[0], weights[1], weights[2]], device=labels.device, dtype=logits.dtype)) 
...
final_true_preds.to_csv(f'{repository_id}/subtaskB_val_True_Predictions.csv', encoding = 'utf-8', index = False, header = True, sep =',')
df_metrics.to_csv(f'{repository_id}/subtaskB_val_Results_Metrics.csv', encoding = 'utf-8', index = False, header = True, sep =',')
...
# Write to a JSON file
with open(f'{repository_id}/submission_subtaskB_test.json', 'w') as f:
